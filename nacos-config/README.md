...
- **Nacos 连接**：生产必须能连上 Nacos。使用 docker-compose 时通过环境变量 `NACOS_HOST`、`NACOS_PORT` 传入；若直接 `java -jar` 部署，须设置 `SPRING_CLOUD_NACOS_CONFIG_SERVER_ADDR` 与 `SPRING_CLOUD_NACOS_DISCOVERY_SERVER_ADDR`（如 `服务器IP:8848`），否则会退化为 localhost:8848 导致连不上。
- **common-secret-prod.properties**：生产库/Redis/RabbitMQ/ES 等地址与密码需与服务器环境一致；`prod.ollama.base-url` 若 AI 服务在 Docker 而 Ollama 在宿主机，请改为 `http://host.docker.internal:11434`（Mac/Win）或宿主机内网 IP。
